    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs">
      <div className="bg-white border border-slate-200 rounded-3xl max-w-lg w-full p-6 shadow-2xl space-y-4 max-h-[90vh] overflow-y-auto">
        ...
        <div className="text-center py-4 bg-slate-50 border border-slate-100 rounded-2xl">
          ...
          <div className="text-sm font-bold text-slate-900 mt-1 px-4 truncate">{transaction.title}</div>
          <div className="text-xs text-slate-500 font-medium mt-0.5">{formatDateFull(transaction.date)}</div>
        </div>

        {/* Details Grid */}
        <div className="grid grid-cols-2 gap-3 text-xs">
          <div className="p-3.5 bg-slate-50 border border-slate-100 rounded-2xl space-y-1">
            <div className="text-[10px] text-slate-400 uppercase font-bold">Kategori</div>
            <div className="font-bold text-slate-900">{transaction.category}</div>
          </div>

          <div className="p-3.5 bg-slate-50 border border-slate-100 rounded-2xl space-y-1">
            <div className="text-[10px] text-slate-400 uppercase font-bold">Akun / Rekening</div>
            <div className="font-bold text-slate-900 truncate">{acc?.name || 'Rekening'}</div>
          </div>

          {destAcc && (
            <div className="col-span-2 p-3.5 bg-slate-50 border border-slate-100 rounded-2xl space-y-1">
              <div className="text-[10px] text-slate-400 uppercase font-bold">Akun Tujuan</div>
              <div className="font-bold text-indigo-600">{destAcc.name}</div>
            </div>
          )}

          <div className="p-3.5 bg-slate-50 border border-slate-100 rounded-2xl space-y-1">
            <div className="text-[10px] text-slate-400 uppercase font-bold">Sumber Input</div>
          ...
          <div className="p-3.5 bg-slate-50 border border-slate-100 rounded-2xl space-y-1">
            <div className="text-[10px] text-slate-400 uppercase font-bold">Metode Bayar</div>
        ...
        {transaction.notes && (
          <div className="p-3.5 bg-slate-50 border border-slate-100 rounded-2xl text-xs space-y-1">
            <div className="text-[10px] text-slate-400 uppercase font-bold">Catatan</div>
            <div className="text-slate-700 leading-relaxed font-medium">{transaction.notes}</div>
          </div>
        )}

        {transaction.tags && transaction.tags.length > 0 && (
          <div className="flex flex-wrap items-center gap-1.5 pt-1">
            <span className="text-[11px] text-slate-400 font-bold mr-1">Tags:</span>
            {transaction.tags.map((tag, i) => (
              <span
                key={i}
                className="px-2.5 py-0.5 bg-slate-100 text-slate-700 rounded-full text-[11px] font-bold border border-slate-200"
              >
                #{tag}
              </span>
            ))}
              ...
              {transaction.receiptItems.map((item, idx) => (
                <div key={idx} className="py-1.5 flex items-center justify-between">
                  <div>
                    ...
                    <span className="text-[10px] text-slate-500 ml-2">({item.quantity}x)</span>
                  </div>
        ...
        <div className="flex items-center justify-between pt-3 border-t border-slate-100">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="px-4 py-2 rounded-xl text-xs font-bold bg-slate-100 hover:bg-slate-200 text-slate-700 border border-slate-200 transition-colors flex items-center gap-1.5 cursor-pointer"
            >
