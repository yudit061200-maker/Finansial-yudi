        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h1 className="text-xl font-bold text-slate-900 tracking-tight">Scan Struk & Nota Belanja</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-5 space-y-4">
          ...
          <div className="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm space-y-4">
            <h2 className="text-sm font-bold text-slate-900">Unggah Foto Struk</h2>

            <label className="border-2 border-dashed border-slate-200 hover:border-indigo-500 bg-slate-50/70 hover:bg-indigo-50/30 rounded-2xl p-6 flex flex-col items-center justify-center cursor-pointer transition-colors text-center group block">
              <div className="w-12 h-12 rounded-2xl bg-indigo-50 text-indigo-600 flex items-center justify-center mb-3 group-hover:scale-105 transition-transform shadow-xs">
                <Camera className="w-6 h-6" />
              </div>
              <span className="text-xs font-bold text-slate-800">Klik untuk ambil foto / upload struk</span>
              <span className="text-[11px] text-slate-400 mt-1">Mendukung JPG, PNG, WEBP, atau screenshot</span>
              <input
                type="file"
                accept="image/*"
                capture="environment"
            ...
            {selectedImage && (
              <div className="p-3.5 bg-slate-50 border border-slate-200 rounded-2xl space-y-2">
                ...
                <div className="h-44 w-full bg-white rounded-xl overflow-hidden flex items-center justify-center border border-slate-200 p-2">
                  <img
                    src={selectedImage}
                    alt="Receipt preview"
          ...
          <div className="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm space-y-3">
            <div className="flex items-center justify-between">
              ...
              <span className="text-[10px] text-indigo-600 font-bold bg-indigo-50 px-2 py-0.5 rounded-full">Test 1-Klik</span>
            </div>
            ...
            <div className="space-y-2">
              {SAMPLE_RECEIPTS.map((sample) => (
                <button
                  ...
                  className="w-full p-3.5 bg-slate-50 hover:bg-indigo-50/50 border border-slate-100 hover:border-indigo-200 rounded-2xl transition-colors flex items-center justify-between text-left group cursor-pointer shadow-xs"
                >
                  <div className="flex items-center gap-3 min-w-0">
                    <div className="w-8 h-8 rounded-xl bg-white border border-slate-200 flex items-center justify-center text-slate-600 group-hover:text-indigo-600 shrink-0 shadow-xs">
                      <Receipt className="w-4 h-4" />
                    </div>
                    <div className="truncate">
                      <div className="text-xs font-bold text-slate-900 truncate">{sample.title}</div>
                      <div className="text-[11px] text-slate-500 font-medium">{sample.items.length} items • {sample.category}</div>
                    </div>
                  </div>
                  <div className="text-xs font-bold text-slate-900 shrink-0 ml-2">
        ...
        <div className="lg:col-span-7">
          <div className="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm space-y-5">
            <div className="flex items-center justify-between pb-3 border-b border-slate-100">
              <div>
                <h2 className="text-base font-bold text-slate-900">Hasil Ekstraksi & Rincian Belanja</h2>
                <p className="text-xs text-slate-500 font-medium">Verifikasi item dan simpan ke pencatatan keuangan</p>
              </div>
            ...
            {isScanning ? (
              <div className="py-16 flex flex-col items-center justify-center text-center space-y-3">
                ...
                <div className="text-sm font-bold text-slate-900">Gemini 3.7 Flash Memproses Struk</div>
                <div className="text-xs text-indigo-600 font-semibold">{scanStep}</div>
              </div>
            ) : !parsedData ? (
              <div className="py-16 flex flex-col items-center justify-center text-center text-slate-400 space-y-2">
                <Receipt className="w-10 h-10 text-slate-300" />
                <p className="text-xs font-semibold text-slate-700">Belum ada struk yang dipindai.</p>
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 p-4 bg-slate-50 border border-slate-100 rounded-2xl text-xs">
                  <div>
                    ...
                    <div className="font-bold text-slate-900 mt-0.5 truncate">{parsedData.merchant}</div>
                  </div>
                  <div>
                    <div className="text-[10px] text-slate-400 uppercase font-bold flex items-center gap-1">
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between px-1">
                    ...
                    <span className="text-[10px] text-slate-400 font-medium">Dapat diedit sebelum disimpan</span>
                  </div>
                  ...
                  <div className="divide-y divide-slate-100 bg-slate-50 border border-slate-200 rounded-2xl overflow-hidden">
                    {parsedData.items.map((item, idx) => (
                      <div
                        key={idx}
                        className="p-3.5 flex items-center justify-between gap-3 text-xs hover:bg-white transition-colors"
                      >
                        <div className="flex-1 min-w-0">
                          <input
                            ...
                            className="w-full bg-transparent font-bold text-slate-900 focus:outline-none border-b border-transparent focus:border-indigo-500 text-xs"
                          />
                          <div className="flex items-center gap-2 mt-1 text-[11px] text-slate-500">
                            ...
                            <input
                              ...
                              className="w-10 bg-white border border-slate-200 rounded px-1 text-center text-slate-900 font-bold"
                            />
                ...
                <div className="p-4 bg-slate-50 border border-slate-100 rounded-2xl space-y-2 text-xs">
                  ...
                  <div className="flex items-center justify-between text-sm font-bold text-slate-900 pt-2 border-t border-slate-200">
                    ...
                    <span className="text-indigo-600 text-base font-black">{formatRupiah(parsedData.total)}</span>
                  </div>
                </div>
                ...
                {saveSuccess ? (
                  <div className="p-3.5 bg-emerald-50 border border-emerald-200 rounded-2xl text-xs font-bold text-emerald-800 flex items-center justify-center gap-2 shadow-xs">
                  ...
                  <button
                    ...
                    className="w-full py-3 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-xl text-xs transition-all shadow-md shadow-indigo-100 flex items-center justify-center gap-2 active:scale-98 cursor-pointer"
                  >
